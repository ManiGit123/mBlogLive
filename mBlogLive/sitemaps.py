...
from blog_listing.models import MyImage, NewsAuthor
...
class CombinedSitemap(Sitemap):
    ...
    def items(self):
        ...
        news_author = list(NewsAuthor.objects.all())
        my_mage = list(MyImage.objects.all())

        # Static URLs (add your view names)
        static_urls = [
            "blog_listing:home_page",
            "blog_listing:about_us",
            "blog_listing:contact_us",
            "blog_listing:privacy_policy",
            "blog_listing:terms_of_service",
        ]

        return wagtail_pages + static_urls + news_author + my_mage
    ...
    def location(self, item):
        ...
        elif isinstance(item, NewsAuthor):  # Django model instance
            # Use your actual detail view name here
            return reverse("blog_listing:author_detail", kwargs={"slug": item.slug})
